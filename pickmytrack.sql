...
--
-- Host: 127.0.0.1
-- Generation Time: May 10, 2025 at 06:16 PM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `DeleteUserResult` (IN `user_id` INT)   BEGIN
    DELETE FROM results WHERE user_id = user_id;
END$$
...
CREATE TABLE `questions` (
  ...
  `track_assessed` varchar(50) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `results` (
  ...
  `specialization` enum('BA','SM','NT') NOT NULL,
...
INSERT INTO `results` (`id`, `user_id`, `specialization`, `created_at`) VALUES
(32, 34, 'BA', '2025-05-10 15:52:17');
...
CREATE TABLE `track_results` (
  ...
  `most_frequent_answer` varchar(100) NOT NULL,
...
--
-- Indexes for table `track_results`
--
...
--
-- AUTO_INCREMENT for table `track_results`
--
ALTER TABLE `track_results`
...
--
ALTER TABLE `results`
  ADD CONSTRAINT `results_ibfk_1` FOREIGN KEY (`id`) REFERENCES `users` (`id`) ON DELETE CASCADE;
COMMIT;
